...
class OrderBook {
    ...
    static convert(x: OrderBook): Model.OrderBook {
        return {
            asks: x.asks.map(Order.convert),
            bids: x.bids.map(Order.convert),
        }
    }
...
export async function GetOrderBook(fromCurrency: string, toCurrency: string): Promise<Model.OrderBook> {
    ...
    const url = `https://poloniex.com/public?command=returnOrderBook&currencyPair=${fromCurrency}_${toCurrency}&depth=100`
